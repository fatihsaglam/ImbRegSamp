#' @title Random over-undersampling
#'
#' @description \code{ROUS} applies random undersampling and oversampling to
...
#'
#' @details
#' Random over-undersampling (ROUS) can be used as random oversampling (ROS) and
#' random undersampling. Random oversampling copies randomly selected rare
#' instances and addes to data. Random udersampling randomly selects non-rare
...
#'
#' plot(x, y)
#' plot(m_ROUS$x_new, m_ROUS$y_new)
#'
...
ROUS <- function(x,
  ...
  data_syn_lower <-
    generator_ROS(data_rare = data_rare_lower,
                    perc_ov = perc_ov_lower)
  data_syn_upper <-
    generator_ROS(data_rare = data_rare_upper,
